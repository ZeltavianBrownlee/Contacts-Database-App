    ...
    class DataAccess
    {
        ...
        public List<Person> GetPeople(string term)
        {
            ...
            while (data_reader.Read())
            {
                ...
                new_person.active = Convert.ToBoolean(records[7]);
        ...
        public void AddPerson(Person newPerson)
        {
            ...
            string queryStatement = $"INSERT INTO Persons (first_name, last_name, cell_number, work_number, email, notes, active) VALUES ('{newPerson.first_name}', '{newPerson.last_name}', '{newPerson.cell_number}', '{newPerson.work_number}'," +
                $"'{newPerson.email}', '{newPerson.notes}' ,'{newPerson.active}'); SELECT SCOPE_IDENTITY() ";
            SqlCommand sql_command = new SqlCommand(queryStatement, sql_connection);
        ...
        public void UpdatePerson(Person newPerson, int idIndex)
        {
            ...
            //BUILD QUERY
            //string queryStatement = $"UPDATE Persons SET first_name = '{newPerson.first_name}', last_name = '{newPerson.last_name}', cell_number = '{newPerson.cell_number}', work_number = '{newPerson.work_number}', email = '{newPerson.email}', home_address = '{newPerson.home_address}', notes = '{newPerson.notes}' WHERE id LIKE '{idIndex}'";
            //SqlCommand sql_command = new SqlCommand(queryStatement, sql_connection);

            string queryStatement = $"UPDATE Persons SET first_name = '{newPerson.first_name}', last_name = '{newPerson.last_name}', cell_number = '{newPerson.cell_number}', work_number = '{newPerson.work_number}', email = '{newPerson.email}', notes = '{newPerson.notes}' WHERE id LIKE '{idIndex}'";
            SqlCommand sql_command = new SqlCommand(queryStatement, sql_connection);
        ...
        public List<Person> GrabArchivedPersonData(Person newPerson)
        {
            ...
            while (data_reader.Read())
            {
                ...
                new_person.notes = records[7].ToString();
                new_person.active = Convert.ToBoolean(records[8]);
